...
function send(msg) {
	...
	function chatmenu() {
			// 제이쿼리 콘텍스트 메뉴
			var size = '<c:out value="${Flist.size()}"/>';
				...
				$.contextMenu({
							...
							callback : function(key, options) {
								...
								if (key == 'edit') {
									...
									addFriend(receiveUserId);
									return;
								}
								if (key == 'message') {
									var senduserId = $(a).children().text();
									var roomN = a.substring(5, 6)
									messageOne(senduserId, roomN);
									return;
								}
							},
							items : {
	...
	function addFriend(receiveUserId){
		if(userId==""){
			alert("로그인을 해주세요");
			return;
		}
		var agree = confirm("친구추가 하시겠습니까?");
		if (agree) {
			$.ajax({
				method : 'POST',
				url : 'friendRequest',
...
<body>
	...
	<section id="myworks" class="myworks">
		...
		<div id="wrapper">
			<div id="in">
				<!-- 검색 폼 -->
				<form class="right" action="Friends" method="get"
						...
						<div id="main" style="margin: 0 auto;">
							<div class="styled-select blue semi-square" style="float: left;">
								<select name="searchItem">
									<option value="userid"
										${searchItem=='userid'  ? 'selected' : '' }>유저아이디</option>
									<option value="username"
										${searchItem=='username'   ? 'selected' : '' }>유저명</option>
								</select>
							</div>
				...
				<table id="type11" class="board">
					...
					<tbody>
						<c:if test="${empty Flist}">
							<tr>
								<td colspan="5">검색하신 유저가 없습니다.</td>
							</tr>
						</c:if>
						<c:if test="${searchWord == ''}">
							<tr>
								<td colspan="5">친구를 검색해주세요.</td>
							</tr>
						</c:if>
						...
						<c:if test="${searchWord != ''}">
							<c:forEach var="i" begin="0" end="${Flist.size()-1}">
								<tr id="${i }">
									<td class="title" style="width: 400px;">${Flist.get(i).userid}</td>
									<td>${Flist.get(i).username}</td>
									<td class="center">${Flist.get(i).email}</td>
									<td>${Flist.get(i).phone}</td>
								</tr>
							</c:forEach>
						</c:if>
					</tbody>
					...
					<nav aria-label="Page navigation example">
						<ul class="pagination">
							<li class="page-item"><a
								href="Friends?currentPage=${navi.currentPage - navi.pagePerGroup}&searchItem=${searchItem }&searchWord=${searchWord }"><<</a></li>
							<li class="page-item"><a
								href="Friends?currentPage=${navi.currentPage - 1}&searchItem=${searchItem }&searchWord=${searchWord }">Prev</a></li>
							...
							<c:forEach var="page" begin="${navi.startPageGroup}"
								...
								<c:if test="${page != navi.currentPage }">
									<li class="page-item"><a
										href="Friends?currentPage=${page}&searchItem=${searchItem }&searchWord=${searchWord }">${page}</a></li>
								</c:if>
							</c:forEach>
							<li class="page-item"><a
								href="Friends?currentPage=${navi.currentPage + 1}&searchItem=${searchItem }&searchWord=${searchWord }">Next</a></li>
							<li class="page-item"><a
								href="Friends?currentPage=${navi.currentPage + navi.pagePerGroup}&searchItem=${searchItem }&searchWord=${searchWord }">>></a></li>
						</ul>
					</nav>
